...
import com.leia.sdk.views.InputAGSLBinding
 ...
 */
abstract class RendererImpl(val renderer: Renderer, listener: SurfaceTextureReadyCallback?) : InputViewsAsset.Impl(listener) {
    ...
    override fun createAGSLBinding(): InputAGSLBinding? {
        return null
    }

    override fun getPreferredSRGBConfig(): InputGLBinding.SRGBConfig {
        return InputGLBinding.SRGBConfig.linear()
    }
    ...
    class RendererGLBinding(impl: RendererImpl, private val _listener: SurfaceTextureReadyCallback?) : InputGLBinding(impl) {
        ...
        override fun render(p0: InterlacedRenderer, p1: Int, p2: Int, p3: Boolean) {
            val renderStartMs = SystemClock.elapsedRealtime()
            val asset = updateAsset(RendererImpl::class.java) ?: return
            if (this._texture.glId == -1) { return }

            val newSize = Size(p1, p2)
            if (_size != newSize) {
                _surface!!.setDefaultBufferSize(p1, p2)
                asset.renderer.onSurfaceChanged(p1, p2)
                _size = newSize
                _stale = true
            }
            ...
            getPreparedFramebuffer()?.let {
                ...
                p0.GLThread_getInterlacer().apply {
                    setSourceViews(_texture.glId, _texture.glType, 0, 0)
                    doPostProcess(p1, p2)
                }
                recordRender(SystemClock.elapsedRealtime() - renderStartMs)
            }
        }

        private fun recordRender(durationMs: Long) {
            val nowMs = SystemClock.elapsedRealtime()
            if (renderWindowStartMs == 0L) {
                renderWindowStartMs = nowMs
            }
            renderCount += 1
            renderMaxMs = maxOf(renderMaxMs, durationMs)
            val elapsedMs = nowMs - renderWindowStartMs
            if (elapsedMs >= 1000) {
                val fps = renderCount * 1000f / elapsedMs
                Log.i(
                    "SketchfabLeia3D",
                    "CnsdkRender fps=${"%.1f".format(fps)} max=${renderMaxMs}ms size=${_size?.width}x${_size?.height}"
                )
                renderWindowStartMs = nowMs
                renderCount = 0
                renderMaxMs = 0
